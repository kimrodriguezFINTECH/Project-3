...
lottie_url = "https://lottie.host/54821ddd-ab85-48b1-8799-d682041bfa49/YUo6MYtdLV.json"
...
if option == 'Create NFT':
    ...
    if image_option == 'Generate via AI':
        ...
        eventdate = st.date_input("Please enter the date of the event", value=None)
        st.write("Your event is:", eventdate)
        eventtime = st.time_input("Time of the event",value=None)
        ...
        if st.button("Generate Image"):
            prompt = f"Create a poster of {descriptionevent} for an event with title {eventname}, on {eventdate}, at {eventtime} hrs at {eventloc}, {eventaddress}"
        ...
        if st.session_state['image_url']:
            ...
            if 'IpfsHash' in response:
                ...
                for i in range(num_copies):
                    tx_hash = contract.functions.registerTicket(Web3.to_checksum_address(st.session_state['eth_address']), token_uri, price_in_wei, 1).transact({'from': web3.eth.default_account, 'gas': 3000000})
